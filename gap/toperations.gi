    ...
    if not n - 1 in const[1] then
      for x in InputAlphabet(T) do
        ...
        if pos = fail then
          ...
          if nofunc[n][x + 1] <> [] and nofunc[n][x + 1][Size(nofunc[n][x + 1])]=const[2][pos][2] then
...
function(T)
  ...
  EqRelation:= UnorderedTuples(States(T), 2);
  Bad:= [];
  for i in InputAlphabet(T) do
...
[IsTransducer],
function(T)
  local s, statepath, letterpath, currentstate;
  for s in States(T) do;
    statepath := [s];
    letterpath := [0];
    currentstate :=  TransducerFunction(T, [0], s)[2];
    while statepath <> [] do
      if currentstate = s and not Size(TransducerFunction(T, letterpath, s)[1]) = Size(letterpath) then
        return false;
      fi;
      if not currentstate in statepath then
        ...
        while letterpath <> [] and letterpath[Size(letterpath)] = Size(InputAlphabet(T)) do;
          Remove(letterpath);
          Remove(statepath);
          if letterpath <> [] then
            letterpath[Size(letterpath)] := letterpath[Size(letterpath)] + 1;
